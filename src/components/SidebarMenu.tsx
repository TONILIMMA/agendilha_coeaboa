...
export function SidebarMenu({ onClose }: Props) {
  ...
  const { status, name, initials, loaded: badgeLoaded } = useUserBadge();
  ...
  const getSidebarSections = (): SidebarSection[] => {
    const userSections: SidebarSection[] = [
      {
        ...
        items: [
          ...
          { label: "Meus Favoritos", path: "/agenda?view=favorites", icon: Heart },
          { label: "Artistas Locais", path: "/artistas", icon: Users },
        ]
      }
    ];
    ...
    const masterSections: SidebarSection[] = [
      {
        title: "Master",
        icon: Star,
        items: [
    ...
    let result = [...userSections];
    if (isAdmin || isMaster) {
      result = [...result, ...adminSections];
    }
    if (isMaster) {
      result = [...result, ...masterSections];
    }
    return [...result, ...accountSections];
  };
  ...
  const getStatusBadge = () => {
    ...
    return (
      <Badge variant="outline" className={cn("px-2 py-0.5 text-[10px] font-black uppercase tracking-widest gap-1 border animate-in fade-in slide-in-from-top-1", variants[status] || "bg-muted text-muted-foreground border-border")}>
    ...
    <div className="flex flex-col h-full bg-sidebar text-sidebar-foreground border-r border-sidebar-border w-64 shadow-xl">
      ...
      <div className="flex-1 overflow-y-auto px-4 py-6 space-y-8 scrollbar-thin scrollbar-thumb-sidebar-border scrollbar-track-transparent">
        {sections.map((section, idx) => {
          ...
          return (
            <div key={idx} className="space-y-1.5 animate-in fade-in slide-in-from-left-2 duration-300" style={{ animationDelay: `${idx * 100}ms` }}>
              <div className="flex items-center gap-2 px-3 mb-2.5 opacity-80">
                {SectionIcon && <SectionIcon className="h-3 w-3 text-primary/70" />}
