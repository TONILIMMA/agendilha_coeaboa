    ...
    >
      ...
      <div className="p-4 pt-10 space-y-3">
        <div className="flex justify-between items-start">
          ...
          <button className="p-2 rounded-full bg-muted/50 text-muted-foreground hover:bg-primary/10 hover:text-primary transition-all shrink-0">
            <Heart className="h-4 w-4" />
          </button>
        </div>

        <div className="flex items-center gap-2 text-[10px] text-muted-foreground pt-2 border-t border-border/50 overflow-hidden">
          <span className="flex items-center gap-1 truncate">
            <MapPin className="h-2.5 w-2.5" /> {artist.neighborhood}
          </span>
          <span className="shrink-0">•</span>
          <span className="capitalize truncate">{artist.artist_type}</span>
        </div>
      </div>
    </Link>
  );
