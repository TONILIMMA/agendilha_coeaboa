    ...
    SELECT jsonb_build_object(
        ...
        'charts', (
            SELECT jsonb_build_object(
                ...
                'eventsByPeriod', (
                    SELECT jsonb_agg(d) FROM (
                        SELECT to_char(created_at, 'DD/MM') as name, count(*) as value
                        FROM public.submissions
                        WHERE created_at >= v_now - interval '10 days'
        ...
        'rankings', (
            SELECT jsonb_build_object(
                ...
                'topArtists', (
                    SELECT jsonb_agg(d) FROM (
                        SELECT ap.name, count(f.id) as count
                        FROM public.artist_profiles ap
                        JOIN public.user_favorites f ON ap.id = f.artist_id
                ...
                'topPlaces', (
                    SELECT jsonb_agg(d) FROM (
                        ...
                        JOIN public.user_favorites f ON p.id = f.place_id
                        WHERE f.created_at >= v_cutoff
